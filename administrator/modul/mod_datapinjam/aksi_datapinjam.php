...
<div class=\"content\">
    ...
    if($module=='datapinjam' AND $act=='kembali'){
        $kembali ="UPDATE peminjaman SET status ='Kembali', username = '".$_SESSION['namauser']."' WHERE no_pinjam='$_GET[no_pinjam]'";
    ...
    elseif($module=='datapinjam' AND $act=='panjang'){
        if($lambat > 3){
            echo "<script>alert('Buku yang dipinjam tidak dapat diperpanjang, karena sudah terlambat lebih dari 3 hari.');</script>";
            echo "<meta http-equiv='refresh' content='0; url=http://localhost/msdb/tubes/perpustakaan/administrator/media.php?module=datapinjam'>";
        // header("location:../../media.php?module=".$module);
        }
        else{
         /*   
	...
	$update_tgl_kembali=mysqli_query($koneksidb,"UPDATE peminjaman SET tgl_kembali='$hari_next', username = '".$_SESSION['namauser']."' WHERE no_pinjam='$_GET[no_pinjam]'");
